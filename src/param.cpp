...
void PARAM::ReadFiles (void)
{
	...
	if (!file_snps.empty()) {
	    ...
	    if (!file_anno.empty()) {
         ...
         if (ReadFile_anno (file_anno, snpInfo, mapID2num, indicator_snp, Anum)==false) {error=true;}
	    }
	    else{
...
void PARAM::CheckData (void) {
  ...
  cout<<"## length of snpInfo = "<<indicator_snp.size()<<endl;
  cout<<"## length of snp_pos = "<<snp_pos.size()<<endl;

	//set parameters for BSLMM
	//and check for predict
	if (a_mode==11 || a_mode==12 || a_mode==13) {
		if (a_mode==11) {n_mh=1;}
		if (logp_min==0) {logp_min=-1.0*log((double)snp_pos.size());}
		...
		if (s_max>snp_pos.size()) {s_max=snp_pos.size(); cout<<"s_max is re-set to the number of analyzed SNPs."<<endl;}
		if (s_max<s_min) {cout<<"error! maximum s value must be larger than the minimal value. current values = "<<s_max<<" and "<<s_min<<endl; error=true;}
	}
...
void CreateSnpPosVec(vector<SNPPOS> &snp_pos, vector<SNPINFO> &snpInfo, const vector<bool> &indicator_snp, size_t &Anum)
{
    ...
    snp_pos.clear();
    //indicator_snp size is the size of snpInfo

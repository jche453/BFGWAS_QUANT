...
setwd("/mnt/YangFSS/data/jchen/simulation_20blocks/wkdir_5")
load("parameter.rda")
...
prehyp <- read.table("/mnt/YangFSS/data/jchen/simulation_20blocks/wkdir_5/hypval.current-5", header=F)
a = prehyp[,1]
b = prehyp[,2]
...
png("/mnt/YangFSS/data/jchen/simulation_20blocks/wkdir_5/pi_pos.png")
plot(Annodata$pi)
dev.off()
...
# of casual snps 
nrow(combine[combine$gamma.x > 0 & combine$gamma.y > 0.01 , ])
#1
...
combine = merge(geno_final, Annodata[c(1:3,8:9)], by.x = "ID", by.y = "SNP")
cc = cor(t(combine[combine$gamma.x > 0, 6:1899]), t(combine[combine$gamma.y > 0.1, 6:1899])) ^2
cc[cc < 0.3] =0
write.csv(cc, "/mnt/YangFSS/data/jchen/simulation_20blocks/wkdir_5/cc_true_estimate.csv")